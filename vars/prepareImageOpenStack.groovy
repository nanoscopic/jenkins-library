...
OpenstackTypeOptions call(Map parameters = [:]) {
    ...
    timeout(10) {
        withCredentials([file(credentialsId: options.openrcCredentialId, variable: 'OPENRC')]) {
            ...
            options.image = sh(script: "set -o pipefail; set +x; source $OPENRC; openstack image list --property caasp-channel='${options.channel}' --property caasp-version='2.0' -c Name -f value | sort -r -V | head -n1 | tr -d \"\n\"", returnStdout: true)
        }
    }
